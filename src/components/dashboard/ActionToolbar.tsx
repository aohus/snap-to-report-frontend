  ...
  return (
    <header className="bg-white/90 border-b border-slate-200 px-4 py-3 md:px-6 flex items-center justify-between flex-shrink-0 z-40 backdrop-blur-xl sticky top-0 shadow-subtle">
      <div className="flex items-center gap-3 md:gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate('/')} className="rounded-md hover:bg-slate-100 h-10 w-10">
          <ArrowLeft className="w-6 h-6 text-slate-600" />
        </Button>
        <div className="p-2 bg-primary rounded-md shadow-subtle hidden md:block">
          <LayoutGrid className="w-5 h-5 text-primary-foreground" />
        </div>
        <div 
          className="group flex items-center cursor-pointer hover:bg-slate-50 px-2 py-1 rounded-md transition-all h-10"
          onClick={onEditJob}
        >
          <div className="flex items-center gap-2">
            <h1 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight truncate max-w-[200px] md:max-w-none">
              {job.title}
            </h1>
